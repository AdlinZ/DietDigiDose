...
export class SqliteHouseholdsRepository implements HouseholdsRepository {
  ...
  async diningSupply(userId: number,householdId: number,target: { planId: string; itemId: string; version: number },totalServings: number,recipeFingerprint: string) {
    return this.database.transaction(() => {
      ...
      const plans = this.database.prepare(`SELECT i.id,i.user_id,i.plan_id,i.version,i.planned_date,i.dining_json,i.recipe_id,r.title AS recipe_title,r.description AS recipe_description,r.ingredients_json AS recipe_ingredients,r.serving_size AS recipe_yield,
