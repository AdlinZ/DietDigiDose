...
本地或其他部署可使用 `pnpm --dir server worker -- --task=intervention-scan` 和 `--task=intervention-delivery` 分别启动。显式的 `WORKER_INTERVAL_MS` 会覆盖代码默认间隔，启用前应检查旧环境是否仍配置为一小时。扫描每批最多处理25个账号，每个账号结束后持久保存游标，下一轮接续并在末尾回绕。异常终止时可重扫尚未保存的位置，候选来源键去重；失败账号完成本次尝试后仍推进游标。大账号量或单账号长时间读取时，不能仅依据一分钟间隔宣称扫描时效已达标。
