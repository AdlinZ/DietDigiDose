...
type Overview = { activeDevices: number; enabledUsers: number; campaigns: Campaign[]; automatic: Automatic[]; metrics: NotificationMetrics };
...
function Metric({ label, value, danger = false }: { label: string; value: number; danger?: boolean }) { return <div className="rounded-xl bg-gray-50 p-4"><div className="text-xs text-text-muted">{label}</div><div className={`mt-1 text-xl font-bold ${danger && value > 0 ? 'text-red-600' : 'text-text-main'}`}>{value}</div></div>; }
