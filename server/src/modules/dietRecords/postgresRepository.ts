...
export class PostgresDietRecordsRepository implements DietRecordsRepository {
  ...
  private async productionBefore(client: PoolClient, userId: number, input: PreparedCookingCompletion) {
    ...
    if (production.plan_item_id) {
      const row = (await client.query("SELECT i.version,i.status,i.recipe_id,i.dining_json FROM meal_plan_items i JOIN meal_plans p ON p.id=i.plan_id WHERE i.id=$1 AND i.user_id=$2 AND i.deleted_at IS NULL AND p.deleted_at IS NULL FOR UPDATE OF i", [production.plan_item_id, userId])).rows[0];
