...
export function nextBuildRelease(release, snapshots, now = new Date()) {
  ...
  if (buildNumber <= release.buildNumber) throw new Error("Build date precedes the existing release; refusing to reuse a build number");
  return { ...release, snapshot, buildNumber };
}

export function prepareBuild({ publish = false, directory = root } = {}) {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    const next = nextBuildRelease(release, snapshots);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  prepareBuild({ publish: process.argv.includes("--publish") });
}
