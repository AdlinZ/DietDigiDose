    ...
    <Modal visible={visible} animationType="slide" transparent>
      <View className="flex-1 bg-canvas">
        <KeyboardAvoidingView
          ...
          className="flex-1 bg-canvas overflow-hidden flex-col justify-between"
        >
          {/* Header */}
          <View className="px-5 pt-4 pb-3 flex-row items-center justify-between border-b border-line/60 bg-white/60">
            ...
            <View className="flex-row items-center gap-1.5 bg-brand/10 px-3 py-1 rounded-full">
              <Text className="text-sm font-black text-ink">食光</Text>
              <View className="bg-brand px-1.5 py-0.5 rounded-md">
            ...
            <View className="flex-row items-center gap-2">
              <TouchableOpacity
                ...
                className="w-8 h-8 rounded-full bg-white items-center justify-center border border-line active:opacity-80"
              >
          ...
          >
            {messages.length === 0 ? (
              <View className="items-center pt-6 pb-4">
                ...
                <View className="relative mb-3 items-center justify-center">
                  <View className="w-24 h-24 rounded-full bg-brand/15 items-center justify-center shadow-lg border-2 border-white">
                    <Image
                      source={require("@/assets/shiyu-avatar.jpg")}
                      className="w-20 h-20 rounded-full"
                    />
                  </View>
                  <View className="absolute -bottom-1 -right-1 bg-highlight px-2 py-0.5 rounded-full border border-white shadow-xs">
                    <Text className="text-[9px] font-black text-ink">大厨智能体</Text>
                  </View>
                </View>

                {/* Slogan */}
                <Text className="text-base font-black text-ink tracking-wide mb-1 text-center">
                  每一顿膳食与卡路里，我都能帮你理清楚
                </Text>
                <Text className="text-xs text-copy-muted mb-5">
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 4 }} className="flex-row gap-3">
                  {cardPrompts.map((card, i) => (
                    <TouchableOpacity
                      ...
                      className="w-44 bg-white p-4 rounded-3xl border border-line shadow-xs justify-between active:scale-95 transition-transform"
                    >
                      <View className="w-9 h-9 rounded-2xl bg-background-secondary items-center justify-center mb-3">
                        <FontAwesome6 name={card.icon} size={16} color={card.color} />
                      </View>
                      <View>
                        <Text className="text-xs font-black text-ink leading-4 mb-1">
                          {card.title}
                        </Text>
                        <Text className="text-[10px] text-copy-muted leading-3">
                          {card.subtitle}
                        </Text>
                      </View>
                    </TouchableOpacity>
              ...
              <View className="pt-4">
                {messages.map((msg) => (
                  ...
                  >
                    {msg.sender === "ai" && (
                      <View className="w-8 h-8 rounded-full bg-brand items-center justify-center mr-2 mt-0.5 shadow-xs">
                    ...
                    >
                      ...
                      {/* Action Card */}
                      {msg.actionCard && (
                        <View className="mt-3 bg-canvas p-3 rounded-2xl border border-highlight/60 shadow-xs">
                          <View className="flex-row items-center justify-between mb-1.5 pb-1 border-b border-line">
                            <View className="flex-row items-center gap-1">
                              <FontAwesome6 name="wand-magic-sparkles" size={11} color="#2D6A4F" />
                              <Text className="text-[11px] font-black text-ink">打卡预填确认卡片</Text>
                            </View>
                          ...
                          <Text className="text-xs font-black text-ink mb-1">
                            {msg.actionCard.foodName} ({msg.actionCard.amount})
                          </Text>
                          <Text className="text-[10px] text-copy-muted mb-2">
                      ...
                      {msg.writeConfirmation && (
                        <View className="mt-3 rounded-2xl border border-brand/25 bg-[#F7FAF8] p-3">
                          ...
                          {!msg.writeConfirmation.committed && (
                            <TouchableOpacity onPress={() => handleCommitWriteConfirmation(msg.id, msg.writeConfirmation!)} className="mt-2 rounded-xl bg-brand py-2 items-center">
                        ...
                        <View className="mt-3 bg-amber-500/10 p-3 rounded-2xl border border-amber-500/30 shadow-xs">
                          <View className="flex-row items-center justify-between mb-1.5 pb-1 border-b border-amber-500/20">
                            <View className="flex-row items-center gap-1">
                              ...
                              <Text className="text-[11px] font-black text-ink">缺料采购预警卡片</Text>
                            </View>
                          </View>
                          <Text className="text-xs font-black text-ink mb-1.5">
                          ...
                          <View className="flex-row flex-wrap gap-1 mb-2">
                            {msg.missingCard.missingIngredients.map((item, idx) => (
                              <View key={idx} className="bg-white px-2 py-0.5 rounded-lg border border-amber-500/30 flex-row items-center gap-1">
                                ...
                                <Text className="text-[9px] text-copy-muted">({item.amount})</Text>
                              </View>
                            ))}
                          </View>
                          <TouchableOpacity
                            onPress={() => handleSendMessage(`我冰箱里只有现有食材，请为我用冰箱里的食材替代推荐适合的料理！`)}
                            className="bg-brand py-2 rounded-xl items-center flex-row justify-center gap-1 active:opacity-90"
                          >
                      ...
                      {msg.solutionCards && msg.solutionCards.length > 0 && (
                        <View className="mt-3 gap-2.5">
                          <Text className="text-xs font-black text-brand px-1">
                            推荐的平替解决方案卡片（含完整食材与亮点）：
                          </Text>
                          {msg.solutionCards.map((card) => (
                            <View
                              key={card.id}
                              className="bg-white rounded-2xl p-3 border border-brand/25 shadow-xs"
                            >
                              <View className="flex-row items-center justify-between mb-1.5 gap-2">
                                <View className="bg-brand px-2 py-0.5 rounded-full shrink-0">
                              ...
                              <View className="bg-brand/10 px-2 py-0.5 rounded-lg border border-brand/20 mb-2 flex-row items-center gap-1 self-start">
                                <FontAwesome6 name="fire" size={9} color="#2D6A4F" />
                                <Text className="text-[9px] font-bold text-brand">{card.macros}</Text>
                              </View>

                              <View className="bg-canvas p-2 rounded-xl mb-2.5 border border-line gap-1">
                                <View className="flex-row items-start gap-1">
                                  <FontAwesome6 name="carrot" size={9} color="#2D6A4F" className="mt-0.5" />
                                  <Text className="text-[10px] font-medium text-ink flex-1 leading-relaxed">
                                    {card.ingredients}
                                  </Text>
                                </View>
                                {card.cookingTip ? (
                                  <View className="flex-row items-start gap-1 pt-1 border-t border-line/60">
                                    <FontAwesome6 name="fire-burner" size={9} color="#D4A276" className="mt-0.5" />
                                    <Text className="text-[9px] text-copy-muted flex-1 leading-relaxed">
                                      {card.cookingTip}
                                    </Text>
                                  </View>
                                ) : null}
                              ...
                              <TouchableOpacity
                                ...
                                className="bg-brand py-2 rounded-xl items-center flex-row justify-center gap-1 shadow-2xs active:opacity-90"
                              >
            ...
            {loading && (
              <View className="flex-row items-center gap-2 bg-white p-3 rounded-2xl border border-line self-start ml-2 mb-4 shadow-xs">
          ...
          <View className="bg-canvas px-4 pt-3 pb-6 border-t border-line shadow-lg">
            {voiceStatusText ? (
              <View className="mb-2 px-3 py-1.5 rounded-xl bg-brand/10 border border-brand/20 flex-row items-center justify-between">
            ...
            {showToolsGrid && (
              <View className="bg-white rounded-2xl p-3.5 border border-line mt-2.5 flex-row items-center justify-around shadow-sm">
                <TouchableOpacity
                  onPress={() => {
      ...
      >
        <View className="flex-1 bg-black/50 justify-end">
          <View className="bg-canvas rounded-t-3xl p-5 max-h-[80%] border-t border-line shadow-2xl">
            <View className="flex-row items-center justify-between pb-3 border-b border-line">
              <View className="flex-row items-center gap-2">
                ...
                <Text className="text-base font-black text-ink">历史对话记录</Text>
              </View>
            ...
            <TouchableOpacity
              onPress={handleStartNewChat}
              className="my-3 bg-brand py-3 rounded-2xl flex-row items-center justify-center gap-2 shadow-xs active:opacity-90"
            >
            ...
            <ScrollView className="space-y-2.5 max-h-[400px]">
              {sessions.length === 0 ? (
                <View className="items-center py-8">
                  <Text className="text-xs text-copy-muted">暂无历史对话记录</Text>
                </View>
              ) : (
                sessions.map((s) => (
                  <View
                  ...
                  >
                    ...
                    >
                      <Text className="text-xs font-black text-ink mb-1" numberOfLines={1}>
                        {s.title}
                      </Text>
                    ...
                    <TouchableOpacity
                      ...
                      className="w-8 h-8 rounded-full bg-background-secondary items-center justify-center active:opacity-70"
                    >
