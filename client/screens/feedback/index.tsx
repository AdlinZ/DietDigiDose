  ...
  return (
    <Screen backgroundColor="#FDF8F0" safeAreaEdges={["top", "left", "right"]}>
      <View className="flex-row items-center justify-between border-b border-line bg-canvas px-5 pb-3 pt-4">
        <TouchableOpacity onPress={() => router.back()} accessibilityLabel="返回" className="h-10 w-10 items-center justify-center rounded-full border border-line bg-white shadow-xs">
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 44 }} keyboardShouldPersistTaps="handled">
        <View className="mb-5 rounded-3xl bg-brand p-5">
        ...
        <View className="gap-2">
          {categories.map((item) => {
            ...
            return <TouchableOpacity key={item.key} onPress={() => setCategory(item.key)} accessibilityRole="radio" accessibilityState={{ selected }} className={`flex-row items-center rounded-2xl border p-4 ${selected ? "border-brand bg-brand/10" : "border-line bg-white"}`}>
              <View className={`mr-3 h-10 w-10 items-center justify-center rounded-xl ${selected ? "bg-brand" : "bg-canvas"}`}>
                <FontAwesome6 name={item.icon} size={15} color={selected ? "#fff" : "#2D6A4F"} />
              </View>
              ...
              {selected ? <FontAwesome6 name="circle-check" size={16} color="#2D6A4F" /> : null}
            </TouchableOpacity>;
          })}
        </View>
        {contextLabel ? <View className="mt-5 flex-row items-center rounded-xl bg-[#EAF3EC] px-3 py-2.5"><FontAwesome6 name="paperclip" size={12} color="#2D6A4F" /><Text className="ml-2 flex-1 text-[11px] font-medium text-[#42624C]">{contextLabel}</Text></View> : null}
        <Text className="mb-2 mt-5 px-1 text-xs font-bold text-copy-muted">详细描述</Text>
        <TextInput value={content} onChangeText={setContent} multiline textAlignVertical="top" maxLength={2000} placeholder="请描述你遇到的问题、期待的改进，或需要客服协助的事项…" placeholderTextColor="#A79B8D" className="min-h-40 rounded-2xl border border-line bg-white p-4 text-sm leading-6 text-ink" />
        <Text className="mt-1.5 text-right text-[10px] text-copy-muted">{content.length}/2000</Text>
        <TouchableOpacity disabled={submitting} onPress={() => void submit()} accessibilityLabel="提交反馈" className={`mt-4 items-center rounded-2xl py-3.5 ${submitting ? "bg-brand/50" : "bg-brand active:opacity-85"}`}>
