...
export async function readPostgresDiningSupply(client: PoolClient,userId: number,householdId: number,target: { planId: string; itemId: string; version: number },totalServings: number,recipeFingerprint: string) {
  ...
  const inventory = (await client.query("SELECT id,version,food_name,quantity,expiration_date,is_available FROM household_inventory_items WHERE household_id=$1",[householdId])).rows as Row[];
  const shopping = (await client.query("SELECT id,household_id,version,name,amount,checked,expiration_date,source_plan_item_id,source_demand_key,source_generated_version,deleted_at,transferred_at FROM household_shopping_items WHERE household_id=$1",[householdId])).rows as Row[];
