...
export default function UserProfileScreen() {
  ...
  if (loading) return <Screen backgroundColor="#F7F5EF"><View className="flex-1 items-center justify-center"><ActivityIndicator color="#2D6A4F" /><Text className="mt-3 text-xs text-copy-muted">正在打开个人主页</Text></View></Screen>;
  if (!profile) return <Screen backgroundColor="#F7F5EF"><View className="flex-1 items-center justify-center px-8"><FontAwesome6 name="user-slash" size={28} color="#8B7D6B" /><Text className="mt-4 text-base font-bold text-ink">该用户暂不可查看</Text><TouchableOpacity onPress={() => router.back()} className="mt-5 rounded-2xl bg-brand px-6 py-3"><Text className="font-bold text-white">返回社区</Text></TouchableOpacity></View></Screen>;
