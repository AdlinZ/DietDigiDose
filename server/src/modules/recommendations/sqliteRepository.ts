...
export class SqliteRecommendationsRepository implements RecommendationsRepository {
  ...
  async planningState(userId: number, startDate: string, _endDate: string) {
    return {
      ...
      plans: this.database.prepare("SELECT id,constraints_json FROM meal_plans WHERE user_id=? AND deleted_at IS NULL AND status='active' AND end_date>=?").all(userId,startDate) as Row[],
  ...
  async preferenceOutcomes(userId: number) {
    const production = this.database.prepare("SELECT id,recipe_id,food_name,produced_servings,produced_at,reported_cooking_minutes FROM prepared_meals WHERE user_id=? AND produced_at>=datetime('now','-30 day')").all(userId) as Row[];
